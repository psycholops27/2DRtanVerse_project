...
{  
    private const float BoundSize = 5f;         
    private const float MovingBoundSize = 3.5f; 
    ...
    void Start()
    {
        ...
        {
            bestScore = ScoreManager.instance.highScoreTS;
            bestCombo = ScoreManager.instance.bestComboTS;
        }
        else
        {
        ...
        Spawn_Block();  // stackCount = 0
        Spawn_Block();  // stackCount = 1
    }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            else
            {
                ...
                if (ScoreManager.instance!= null)
                {
                    ScoreManager.instance.SetLastScoreTS(stackCount);
                }
            }
        }
    ...
    void CreatRubble(Vector3 pos, Vector3 scale)
    {
    ...
    void UpdateScore()
    {
        if (bestScore < stackCount)
        {
            ...
            if (ScoreManager.instance != null)
            {
                ScoreManager.instance.UpdateHighScoreTS(bestScore, bestCombo);
            }
        }
    }
    ...
    void GameOverEffect()
    {
        ...
        for (int i = 1; i < 20; i++)
        {
            if (childCount < i) break; // 존재하지 않는 i번째 블록을 가져오려다 에러가 나는 것을 방지
